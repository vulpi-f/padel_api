...
const adManager = {
  ...
  showFullScreen() {
    ...
    if (adConfig.durations.fullScreen > 0) {
      this._fsTimeoutId = setTimeout(() => this.hideFullScreen(), adConfig.durations.fullScreen * 1000);
    }
  },
  hideFullScreen() {
    clearTimeout(this._fsTimeoutId);
...
// ===== Configurazione evento fine game =====
// gameEndConfig.event può essere:
//   - 'CNT','RED','BLU' → attende quel singolo press
//   - 'ANY'           → attende qualsiasi press da pulsantiera (CNT, RED o BLU)
//   - numero (es. 5)  → timer in secondi per chiudere automaticamente
const gameEndConfig = {
  event: 'ANY'  // puoi cambiare in 'RED', 'BLU', 'ANY' o 5 (secondi)
};

let _gameCloseTimeout = null;

/** schedule (e pulisce) il timeout di chiusura game */
function scheduleAutoClose() {
  clearTimeout(_gameCloseTimeout);
  if (typeof gameEndConfig.event === 'number') {
    _gameCloseTimeout = setTimeout(() => {
      if (pendingGameClose) closeGame();
    }, gameEndConfig.event * 1000);
  }
}
...
// Mini‑scoreboard elements (overlay ad)
const miniTeam1El       = document.getElementById('mini-team1-score');
const miniTeam1GamesEl  = document.getElementById('mini-team1-games');
const miniTeam1SetsEl   = document.getElementById('mini-team1-sets');
const miniTeam2El       = document.getElementById('mini-team2-score');
const miniTeam2GamesEl  = document.getElementById('mini-team2-games');
const miniTeam2SetsEl   = document.getElementById('mini-team2-sets');

// ===== Config eventi =====
const setCloseConfig = {};
if (typeof gameEndConfig.event === 'string' && gameEndConfig.event !== 'ANY') {
  setCloseConfig.event = `SHORT PRESS : ${gameEndConfig.event}`;
}
...
function updateScoreUI(){
  ...
  // -------- sincronizza il mini‑scoreboard --------
  miniTeam1El.textContent      = Team1ScoreEl.textContent;
  miniTeam1GamesEl.textContent = Team1GameScoreEl.textContent;
  miniTeam1SetsEl.textContent  = Team1SetScoreEl.textContent;
  miniTeam2El.textContent      = Team2ScoreEl.textContent;
  miniTeam2GamesEl.textContent = Team2GameScoreEl.textContent;
  miniTeam2SetsEl.textContent  = Team2SetScoreEl.textContent;
}
...
function checkGameWin(){
  ...
  if (!willWinSet || !adConfig.triggers.onSetEnd) {
...
function connect(){
  ...
  ws.onmessage = e => {
    ...
    // ── CONFERMA CAMBIO CAMPO ──
    if (isChangeoverPending) {
      const confirmCode = setCloseConfig.event;
      // Se ho un codice esplicito, lo confronto,
      // altrimenti (gameEndConfig.event==='ANY') accetto qualsiasi comando diverso dall'undo
      if ((confirmCode && text === confirmCode) ||
          (gameEndConfig.event === 'ANY' && text !== undoConfig.event)) {
        doChangeover();
      }
      return;
    }

    // ── SCELTA SERVIZIO INIZIALE ──
    ...
    if (pendingGameClose) {
      if (typeof gameEndConfig.event !== 'number') {
        if (gameEndConfig.event === 'ANY') {
          // chiudo su qualsiasi comando diverso dall'undo
          if (text !== undoConfig.event) {
            closeGame();
          }
        } else if (text === setCloseConfig.event) {
          // chiudo solo sul comando configurato (CNT/RED/BLU)
          closeGame();
        }
      }
      return;
